...
const KPI = ({label, value, color=G.text, sub}) => (
  <div style={{...glassCard(), textAlign:"center"}}>
    <div style={{fontSize:9,color:G.sub,letterSpacing:"2px",marginBottom:4,textTransform:"uppercase"}}>{label}</div>
    <div style={{fontSize:20,fontWeight:700,color,fontFamily:"monospace"}}>{value}</div>
    {sub && <div style={{fontSize:9,color:G.dim,marginTop:2}}>{sub}</div>}
  </div>
);
...
const MetricCard = ({label, value, suffix="", good, bad}) => {
  ...
  return (
    <div style={{
      ...
      <div style={{fontSize:8,color:G.sub,letterSpacing:"1px",marginBottom:4,textTransform:"uppercase"}}>{label}</div>
...
const MetricGroup = ({label, children}) => (
  <div style={{marginBottom:14}}>
    <div style={{fontSize:9,color:G.sub,letterSpacing:"2px",marginBottom:8,textTransform:"uppercase",
  ...
  return (
    <div style={{display:"flex",alignItems:"center",gap:3}}>
      {Array.from({length:8}).map((_,i) => (
        <div key={i} style={{width:5,height:13,borderRadius:3,
          background:i<score?color:"rgba(255,255,255,0.1)",
          boxShadow:i<score?`0 0 6px ${color}40`:undefined}} />
      ))}
      <span style={{fontSize:10,color,fontFamily:"monospace",marginLeft:3}}>{score}/8</span>
    </div>
  );
};
    ...
    <div>
      <input value={search} onChange={e=>setSearch(e.target.value)}
        ...
        style={{
          ...
          border:`1px solid ${G.border}`,
          borderRadius:30,color:G.text,
          padding:"7px 16px",fontSize:11,width:220,outline:"none",
              ...
              return (
                <tr key={s.ticker}
                  ...
                  <td style={{padding:"10px 12px"}}>
                    ...
                    <div style={{fontSize:9,color:G.sub,marginTop:1,maxWidth:160,
                      overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{s.name}</div>
                  </td>
                  <td style={{padding:"10px 12px"}}><SectorPill sector={s.sector}/></td>
                  <td style={{padding:"10px 12px",textAlign:"right",fontFamily:"monospace",
                    fontWeight:700,color:G.text}}>${fmt(s.price)}</td>
                  <td style={{padding:"10px 12px",textAlign:"right",fontFamily:"monospace",
                    color:s.pe_ttm!=null&&s.pe_ttm<25?G.green:s.pe_ttm>50?G.red:G.text}}>
                    {s.pe_ttm!=null?`${fmt(s.pe_ttm,1)}x`:"—"}</td>
                  <td style={{padding:"10px 12px",textAlign:"right",fontFamily:"monospace",
                    color:s.peg!=null&&s.peg<1?G.green:s.peg>2?G.red:G.text}}>
                    {s.peg!=null?fmt(s.peg,2):"—"}</td>
                  <td style={{padding:"10px 12px",textAlign:"right",fontFamily:"monospace",
                    color:s.gross_mgn!=null&&s.gross_mgn>50?G.green:G.text}}>
                    {s.gross_mgn!=null?`${fmt(s.gross_mgn,1)}%`:"—"}</td>
                  <td style={{padding:"10px 12px",textAlign:"right",fontFamily:"monospace",
                    color:s.roe!=null&&s.roe>15?G.green:s.roe<5?G.red:G.text}}>
                    {s.roe!=null?`${fmt(s.roe,1)}%`:"—"}</td>
                  <td style={{padding:"10px 12px",textAlign:"right",fontFamily:"monospace",
                    color:pColor(s.eps_this_y)}}>
                    {s.eps_this_y!=null?fmtP(s.eps_this_y):"—"}</td>
                  <td style={{padding:"10px 12px",textAlign:"right",fontFamily:"monospace",
                    color:pColor(s.upside)}}>
                    {s.upside!=null?fmtP(s.upside):"—"}</td>
                  <td style={{padding:"10px 12px",textAlign:"right"}}>
                    <ScoreBar score={s.score||0}/></td>
  ...
  const CustomTooltip=({active,payload,label})=>{
    ...
    return (
      <div style={{...glassCard({padding:"8px 12px"}),fontSize:11}}>
        <div style={{color:G.sub,marginBottom:2}}>{label}</div>
        <div style={{color:col,fontFamily:"monospace"}}>${payload[0]?.value}</div>
      </div>
    );
  };
  ...
  return (
    <div>
      <GlassCard style={{marginBottom:12,borderColor:`${col}40`,
        ...
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
          <div>
            ...
            <div style={{fontSize:12,color:G.sub,marginTop:2}}>{stock.name}</div>
          ...
          <div style={{textAlign:"right"}}>
            <div style={{fontSize:30,fontWeight:900,fontFamily:"monospace",color:G.text}}>
              ${fmt(stock.price)}
            </div>
            {up!=null&&(
              <div style={{fontSize:11,color:up>0?G.green:G.red,marginTop:2}}>
                Target ${fmt(stock.target)}  ({fmtP(up)})
              </div>
            )}
            {stock.earnings&&(
            ...
            <ResponsiveContainer width="100%" height={180}>
              <LineChart data={histData} margin={{top:5,right:10,left:-25,bottom:0}}>
                ...
                <Line type="monotone" dataKey="price" stroke={col} strokeWidth={2}
                  dot={false} activeDot={{r:4,fill:col,stroke:"transparent"}}/>
              </LineChart>
            </ResponsiveContainer>
          ):(
            <div style={{color:G.sub,fontSize:11,padding:"20px 0",textAlign:"center"}}>
              Τρέξε python fetch_data.py για ιστορικό
            </div>
          )}
        </GlassCard>
            ...
            {SCORE_CRITERIA.map(c=>{
              ...
              return (
                <div key={c.k} style={{display:"flex",alignItems:"center",gap:8,fontSize:11}}>
                  <span style={{
                    color:met?G.green:G.red,
                    textShadow:met?`0 0 8px ${G.green}`:undefined,
                    minWidth:16,fontFamily:"monospace"
                  }}>{met?"✓":"✗"}</span>
                  <span style={{color:met?G.text:G.sub,flex:1}}>{c.label}</span>
                  <span style={{color:G.dim,fontFamily:"monospace",fontSize:10}}>
                    {val!=null?fmt(val,1):"—"}
                  </span>
                </div>
              );
        ...
        <GlassCard style={{marginBottom:12}}>
          ...
          <ResponsiveContainer width="100%" height={220}>
            <BarChart data={cmpData} margin={{top:10,right:20,left:-10,bottom:40}}>
              <XAxis dataKey="name" tick={{fill:G.sub,fontSize:9}} tickLine={false} axisLine={false}
                angle={-30} textAnchor="end"/>
              <YAxis tick={{fill:G.sub,fontSize:9}} tickLine={false} axisLine={false}/>
              <Tooltip contentStyle={{background:"rgba(15,12,41,0.95)",border:`1px solid ${G.border}`,
                borderRadius:10,fontFamily:"monospace",fontSize:11,backdropFilter:"blur(20px)"}}/>
              <Legend wrapperStyle={{color:G.sub,fontSize:10,paddingTop:10}}/>
              <Bar dataKey={stock.ticker} fill={col} radius={[4,4,0,0]} opacity={0.85}/>
              <Bar dataKey={`Avg ${stock.sector}`} fill="rgba(255,255,255,0.2)" radius={[4,4,0,0]}/>
            </BarChart>
          </ResponsiveContainer>
        </GlassCard>
      )}
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
        ...
        <GlassCard>
          <div style={{fontSize:9,color:G.sub,letterSpacing:"2px",marginBottom:10,textTransform:"uppercase"}}>Ownership & Performance</div>
...
const SectorView = ({stocks}) => {
  ...
  const kpis=[
    {k:"pe_ttm",l:"P/E TTM"},{k:"ps",l:"P/S"},
    {k:"gross_mgn",l:"Gross%"},{k:"net_mgn",l:"Net%"},
    {k:"roe",l:"ROE%"},{k:"eps_this_y",l:"EPS Yr%"},
    ...
    <div>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:16}}>
        <GlassCard>
          ...
          <ResponsiveContainer width="100%" height={220}>
            <BarChart data={roeData} margin={{top:10,right:10,left:-20,bottom:55}}>
              <XAxis dataKey="name" tick={{fill:G.sub,fontSize:8}} tickLine={false} axisLine={false} angle={-35} textAnchor="end"/>
              <YAxis tick={{fill:G.sub,fontSize:8}} tickLine={false} axisLine={false}/>
        ...
        <GlassCard>
          <div style={{fontSize:9,color:G.sub,letterSpacing:"2px",marginBottom:10,textTransform:"uppercase"}}>Avg Gross Margin% by Sector</div>
          <ResponsiveContainer width="100%" height={220}>
            <BarChart data={mgnData} margin={{top:10,right:10,left:-20,bottom:55}}>
              <XAxis dataKey="name" tick={{fill:G.sub,fontSize:8}} tickLine={false} axisLine={false} angle={-35} textAnchor="end"/>
              <YAxis tick={{fill:G.sub,fontSize:8}} tickLine={false} axisLine={false}/>
              <ReferenceLine y={40} stroke={G.yellow} strokeDasharray="3 3" opacity={0.5}/>
              <Tooltip contentStyle={ttStyle} formatter={v=>[`${v}%`,""]}/>
              <Bar dataKey="value" radius={[4,4,0,0]} fill="url(#mgnGrad)"/>
              <defs>
                <linearGradient id="mgnGrad" x1="0" y1="0" x2="0" y2="1">
              ...
              <tr>
                {["Sector","#",...kpis.map(k=>k.l)].map(h=>(
                  <th key={h} style={{padding:"8px 12px",textAlign:h==="Sector"?"left":"right",
                    fontSize:9,color:G.sub,letterSpacing:"1.5px",fontWeight:400,
                    borderBottom:`1px solid ${G.border}`,textTransform:"uppercase"}}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
                ...
                <tr key={row.sector} style={{borderBottom:`1px solid ${G.border}`,
                  ...
                  <td style={{padding:"9px 12px",textAlign:"left"}}>
                    <span style={{color:sc(row.sector),fontFamily:"monospace",fontWeight:700,
                      textShadow:`0 0 10px ${sc(row.sector)}40`}}>{row.sector}</span>
                  </td>
                  <td style={{padding:"9px 12px",textAlign:"right",color:G.sub}}>{row.count}</td>
                  {kpis.map(({l})=>{
                    const v=row[l];
                    const isG=(l==="ROE%"&&v>15)||(l==="Gross%"&&v>40)||(l==="EPS Yr%"&&v>20);
                    const isB=((l==="ROE%"||l==="Net%")&&v<0)||(l==="D/Eq"&&v>2);
                    return (
                      <td key={l} style={{padding:"9px 12px",textAlign:"right",fontFamily:"monospace",
                        color:isG?G.green:isB?G.red:G.text}}>{v!=null?v:"—"}</td>
                    );
                  })}
                </tr>
              ))}
...
export default function App() {
  ...
  const handleSelect=s=>{ setSelected(s); setTab("detail"); };

  const tabs=[
    {id:"watch",   label:"⭐  Watchlist",          color:"#93c5fd"},
    {id:"growth",  label:"🚀  Growth Candidates",  color:"#6ee7b7"},
    {id:"sectors", label:"📊  Sector Averages",    color:"#c4b5fd"},
    {id:"detail",  label:"📋  Stock Analysis",     color:"#fcd34d"},
  ];

  const nowStr=now.toLocaleDateString("el-GR",{day:"2-digit",month:"2-digit",year:"numeric"})
  ...
  return (
    <div style={{
      ...
      <div style={{position:"fixed",top:200,right:-100,width:350,height:350,
        borderRadius:"50%",background:"rgba(16,185,129,0.08)",
        filter:"blur(80px)",pointerEvents:"none",zIndex:0}}/>
      <div style={{position:"fixed",bottom:-50,left:"40%",width:300,height:300,
        ...
        <div style={{
          ...
          <div style={{display:"flex",alignItems:"center",gap:12}}>
            <div style={{
              ...
              background:"#6ee7b7",
              boxShadow:"0 0 12px #6ee7b7, 0 0 24px #6ee7b760",
              animation:"pulse 2s infinite",
            }}/>
            <span style={{fontSize:13,color:G.text,letterSpacing:"3px",fontWeight:600}}>
              STOCK SCREENER
            </span>
            {data&&(
              <span style={{fontSize:10,color:G.dim,marginLeft:4}}>
                Data: {data.updated}
              </span>
            )}
          </div>
          <div style={{display:"flex",alignItems:"center",gap:12}}>
            <span style={{fontSize:10,color:G.sub}}>{nowStr}</span>
            <button onClick={loadData} disabled={loading} style={{
              background:"rgba(147,197,253,0.1)",
              border:"1px solid rgba(147,197,253,0.25)",
              color:"#93c5fd",borderRadius:30,
        ...
        <div style={{
          ...
          {tabs.map(t=>(
            <button key={t.id} onClick={()=>setTab(t.id)} style={{
              ...
              padding:"11px 20px",fontSize:10,letterSpacing:"1.5px",
              cursor:"pointer",fontFamily:"inherit",
              transition:"all .2s",fontWeight:tab===t.id?600:400,
            }}>{t.label}</button>
          ))}
        </div>
        ...
        <div style={{padding:"20px 24px"}}>
          ...
          {!data&&!loading&&!error&&(
            <div style={{textAlign:"center",padding:60,color:G.sub}}>
              Τρέξε <code style={{color:"#93c5fd"}}>python fetch_data.py</code> και μετά πάτα Refresh
            </div>
          )}

          {data&&tab==="watch"&&(()=>{
            const w=data.watch||[];
            const avgPE=w.length?w.reduce((s,x)=>s+(x.pe_ttm||0),0)/w.filter(x=>x.pe_ttm).length:0;
            const avgGr=w.length?w.reduce((s,x)=>s+(x.eps_this_y||0),0)/w.filter(x=>x.eps_this_y).length:0;
            ...
            return (
              <div>
                ...
                <GlassCard>
                  ...
                  <StockTable stocks={w} onSelect={handleSelect}/>
                </GlassCard>
              </div>
            );
          })()}
          ...
          {data&&tab==="growth"&&(()=>{
            ...
            const agm=g.length?g.reduce((s,x)=>s+(x.gross_mgn||0),0)/g.filter(x=>x.gross_mgn).length:0;
            const any_=g.length?g.reduce((s,x)=>s+(x.eps_next_y||0),0)/g.filter(x=>x.eps_next_y).length:0;
            const aup=g.filter(x=>x.upside!=null).length?
              g.filter(x=>x.upside!=null).reduce((s,x)=>s+x.upside,0)/g.filter(x=>x.upside!=null).length:0;

            const secData=Object.entries(
              g.reduce((acc,s)=>{ acc[s.sector]=(acc[s.sector]||0)+1; return acc; },{})
            ).sort((a,b)=>b[1]-a[1]).slice(0,8).map(([name,count])=>({name,count}));

            const scData=Object.entries(
              g.reduce((acc,s)=>{ const sc=String(s.score||0); acc[sc]=(acc[sc]||0)+1; return acc; },{})
            ).sort((a,b)=>Number(a[0])-Number(b[0])).map(([score,count])=>({score,count}));
            ...
            return (
              <div>
                <div style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:12,marginBottom:20}}>
                  ...
                  <KPI label="Avg EPS Next Y" value={fmtP(any_)}          color={G.purple}/>
                  <KPI label="Avg Upside"    value={fmtP(aup)}            color={pColor(aup)}/>
                </div>
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:16}}>
                  <GlassCard>
                    <div style={{fontSize:9,color:G.sub,letterSpacing:"2px",marginBottom:10,textTransform:"uppercase"}}>Sector Breakdown</div>
                    <ResponsiveContainer width="100%" height={180}>
                      <BarChart data={secData} margin={{top:5,right:10,left:-20,bottom:50}}>
                        <XAxis dataKey="name" tick={{fill:G.sub,fontSize:8}} tickLine={false} axisLine={false} angle={-30} textAnchor="end"/>
                        <YAxis tick={{fill:G.sub,fontSize:8}} tickLine={false} axisLine={false}/>
                  ...
                  <GlassCard>
                    <div style={{fontSize:9,color:G.sub,letterSpacing:"2px",marginBottom:10,textTransform:"uppercase"}}>Score Distribution</div>
                    <ResponsiveContainer width="100%" height={180}>
                      <BarChart data={scData} margin={{top:5,right:10,left:-20,bottom:10}}>
                        <XAxis dataKey="score" tick={{fill:G.sub,fontSize:9}} tickLine={false} axisLine={false}/>
                        <YAxis tick={{fill:G.sub,fontSize:9}} tickLine={false} axisLine={false}/>
                        <Tooltip contentStyle={ttStyle}/>
                        <Bar dataKey="count" radius={[4,4,0,0]} fill="url(#scGrad)"/>
                        <defs>
                ...
                <GlassCard>
                  ...
                  <StockTable stocks={g} onSelect={handleSelect}/>
                </GlassCard>
              </div>
            );
          })()}

          {data&&tab==="sectors"&&<SectorView stocks={allStocks}/>}
